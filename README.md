...
過去1年間で **1,557件のコミット**を記録し、約20名のチーム内で3位のコントリビューターとして継続的に開発を推進しました。
特に2025年3月〜5月には月平均300件以上のコミットを達成し、プロジェクトの中核機能である座席選択、機内食選択、オプショナルサービスの実装を担当しました。
コード変更量は **+60,409行 / -36,360行**と大規模な機能開発とリファクタリングの両面で貢献しています。

#### 保守性を重視した品質へのこだわり

コミットの39%がリファクタリング、35%が新機能実装、7%がバグ修正という内訳から分かる通り、
単に機能を作るだけでなく、**保守性の高いコードベースの構築**に継続的に注力しました。
具体的には、ViewModel パターンの適用による UI ロジックとビジネスロジックの分離、
